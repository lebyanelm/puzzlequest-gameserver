...
def request_response(request):
	...
	r = requests.post("https://api.openai.com/v1/chat/completions", headers={ "Authorization": f"Bearer {os.environ['OPENAI_API_KEY']}", "Content-Type": "application/json" },
	      	json={ "model": "gpt-3.5-turbo", "temperature": 1, "messages": [{"role": "user", "content": request}]  })
...
def get_a_word_clue(word):
	return request_response(request=f"Give a clue for the following word: {word}. Rules, don't mention the exact word, be more creative with the clue and make it fun to guess.")

def get_a_word_example(word):
	return request_response(request=f"Give me an example in which the word \"{word}\" can be used. Replace the \"{word}\" with underscores (_) in the response for a guess game.")

def get_part_of_speech(word):
	return request_response(request=f"In a one word answer, whats the part of speech for the following word: {word}?")
...
def status():
	...
	try:
		if CURRENT_SENTENCE.get('is_completed') is None or CURRENT_SENTENCE.get('is_completed') is True:
			...
			for random_sentence_word in random_sentence_words:
				...
				if clues[random_sentence_word]['example']:
					clues[random_sentence_word]['example'].replace(random_sentence_word, '_')
			...
			for line_index, _ in enumerate(text["lines"]):
				for word_index, random_sentence_word in enumerate(random_sentence_words):
					if len(text['lines'][line_index]) < 12 and len(text['lines'][line_index]) + len(random_sentence_word) <= 12:
						if random_sentence_word not in added_words:
							text['lines'][line_index] = [*text['lines'][line_index], *list(random_sentence_word)]
							if len(text['lines'][line_index]) != 12:
								text['lines'][line_index].append(' ')

								# Check if the next word will fit. Else add empty spaces.
								if (word_index != len(random_sentence_words) - 1) and (len(random_sentence_words[word_index - 1]) + len(text['lines'][line_index]) > 12):
									number_of_space = 12 - len(text['lines'][line_index])
									spaces = (" ." * number_of_space).split(".")[0:-1]

									text['lines'][line_index] = [*text['lines'][line_index], *spaces]
